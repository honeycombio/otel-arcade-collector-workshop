...
4. Read through the scaffolding. Each commented block has a hint
...
3. Add an OTTL statement that matches span names starting with a
SQL keyword (`SELECT`, `INSERT`, `UPDATE`, `DELETE`, `CREATE`) and
replaces the whole name with `db.query`:
```yaml
...
```bash
   make local-restart-collector
```
5. Select **◈ Visualizer** and check the Split view. SQL span names
...
3. Add an OTTL statement that sets `attributes["player.id"]` to
`***`, but only when the attribute is present:
```yaml
...
1. Select **⚙ Deploy & Configure** in the app's left navigation.
2. Find the `filter/drop_probes` block. Uncomment it and wire it
into your pipeline processors before `transform/normalize`:
```yaml
...
```bash
   make local-restart-collector
```
4. Confirm no `/health` or `/ready` spans appear in the feed.
...
3. Add an OTTL statement that applies a 128-character limit to
every attribute on a span at once:
```yaml
...
3. Add a `resource` context block and a `delete_key` statement:
```yaml
   - context: resource
     statements:
...
config matches the expected solution and set yourself up for
Workshop 2.
...
- Your config matches the Lab 2 solution
